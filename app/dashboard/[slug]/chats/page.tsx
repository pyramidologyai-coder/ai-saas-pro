...
export default function Dashboard({ params }: { params: { slug: string } }) {
  ...
  async function act(action: string, payload: Record<string, unknown>, done: string) {
    ...
    setActing(key);
    try {
      const endpoint = ["invoice", "invoice_status"].includes(action)
        ? "/api/platform" : "/api/dashboard";
      const res = await fetch(endpoint, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify(endpoint === "/api/platform"
          ? { slug, action, payload }
          : { action, ...payload }),
      });
      const r = await res.json();
    ...
    <Shell>
      ...
      <header style={S.head}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <a href={`/dashboard/${slug}`} style={S.linkQuiet}>← Dashboard</a>
                ...
                <div key={b.id} data-row style={S.row}>
                  ...
                  <div style={S.rightCol}>
                    ...
                    <div style={S.actions}>
                      ...
                      {b.status === "confirmed" && (
                        <button disabled={acting === b.id}
                          onClick={() => act("invoice", { id: b.id }, "Invoice raised. See it in Finance.")}
                          style={{ ...S.act, color: C, borderColor: C }}>Invoice</button>
                      )}
                      <button disabled={acting === b.id}
      ...
      <Assistant slug={slug} color={C} agent={data.business?.agent} />
