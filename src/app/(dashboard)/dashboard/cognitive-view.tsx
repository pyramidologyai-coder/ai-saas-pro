...
export default function CognitiveDashboard({ tenantId, isAgency = false }: { tenantId?: string, isAgency?: boolean }) {
  // SWR: Stale-While-Revalidate for Optimistic UI and Caching
  const endpoint = isAgency 
    ? `/api/bi/metrics?agencyId=${tenantId}` 
    : `/api/bi/metrics?tenantId=${tenantId}`;
  ...
  return (
    <div className="space-y-8 mt-6">
      ...
      {/* 2. Performance Metrics (The Triple Crown) */}
      <div>
        <h3 className="text-lg font-bold mb-4 flex items-center gap-2"><Target size={20} color="var(--accent-primary)"/> أداء العيادة (The Triple Crown)</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
           ...
           <div className="bg-[var(--card-bg)] border border-[var(--glass-border)] p-5 rounded-2xl">
              <p className="text-[var(--text-dim)] text-sm">العوائد المحققة (Revenue Generated)</p>
              <h3 className="text-2xl font-bold mt-1 text-[var(--accent-primary)]">${metrics?.tripleCrown?.revenueGenerated}</h3>
              <p className="text-xs text-[var(--text-dim)] mt-2">إجمالي قيمة حجوزات الـ AI</p>
           </div>
           <div className="bg-[var(--card-bg)] border border-[var(--glass-border)] p-5 rounded-2xl">
              <p className="text-[var(--text-dim)] text-sm">أداء البوت (Resolution Rate)</p>
              <h3 className="text-2xl font-bold mt-1 text-purple-400">{metrics?.tripleCrown?.resolutionRate}</h3>
              <p className="text-xs text-[var(--text-dim)] mt-2">محادثات تم حلها بدون تدخل بشري</p>
           </div>
        </div>
      </div>
      ...
      <div className="bg-[var(--card-bg)] border border-[var(--glass-border)] p-6 rounded-3xl h-80">
         <h3 className="text-sm font-bold mb-6 text-[var(--text-dim)]">مخطط العوائد المحققة بواسطة الذكاء الاصطناعي (AI Profit Engine)</h3>
